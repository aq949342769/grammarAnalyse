...
//系统生成初始变量
interface Grammar { [key: string]: Array<Array<string>> }
export interface TABLE { [key: string]: Set<string> } //只记录了非终结符的
interface ISTONIL { [key: string]: boolean | undefined }
//  枚举类型nil必须放在最后，同时要和对应的字符数组顺序保持一致
// 测试用例1
// enum VN { E0, E1, T0, T1, F };
// enum VT { i, plus, mul, leftBracket, rightBrancket, shur, nil };
let VN_arr: Array<string> = ["E", "E'", "T", "T'", "F"]
let VT_arr: Array<string> = ["i", "+", "*", "(", ")", "#", "nil"]
let G: Array<string> = ["E->TE'", "E'->+TE'", "E'->nil", "T->FT'", "T'->*FT'", "T'->nil", "F->i", "F->(E)"]
let startChar = 'E'
// ------------------------
// 测试用例2
// enum VN { S, A, B, C, D }
...
let isToNil: ISTONIL = {}                   //能推出空字符的布尔向量
let FIRST_table: TABLE = {}           //每个文法符号的first集
let FOLLOW_table: TABLE = {}         //每个非终结符的follow集
let SELECT_table: TABLE = {}         //每个产生式的select集
let FORECAST_TABLE: Array<Array<number>> = []    //预测分析表
let CStack: Array<string> = [];             //分析栈
let cur: number = 0;                       //指向目前读到的字母索引
let SYM: string = '';              //目前读到的符号
...
function getIsToNil(): ISTONIL {
  ...
  while (modify) {
    ...
    for (let i = 0; i < VN_arr.length; i++) {
      let vn = VN_arr[i]
      let productionArr = G_struct[vn]
      if (productionArr.length === 0) continue
      //遍历某一条产生式的右部
      for (let j = 0; j < productionArr.length; j++) {
        let production = productionArr[j]
        for (let k = 0; k < production.length; k++) {
 ...
 * 求出FIRST集
 */
function getSingleSymFIRST(x: string, G_struct: Grammar): Set<string> {
  ...
  let singleSymFIRST: Set<string> = FIRST_table.hasOwnProperty(x) ? FIRST_table[x] : new Set()
 ...
 * 求任意符号的FIRST集
 */
function getAnySymFIRST(str: string, G_struct: Grammar): Set<string> {
 ...
 * 求出FOLLOW集
 */
function getFOLLOW(G_struct: Grammar): TABLE {
  VN_arr.forEach(e => {
    if (e === startChar) {
...
function isChange(size: number, set_: Set<string>): boolean {
  return size !== set_.size
}
/**
 * 求出SECLECT集
 */
function getSELLECT(G_struct: Grammar): TABLE {
  //遍历所有产生式A->a
 ...
 * 分析入口程序
 */
function interpret(target: string) {
  init()
  CStack.push('#')
...
export {
  ...
  FOLLOW_table as follow,
  SELECT_table as select,
